...
public class MemberRestController {
    ...
    public ApiResponse<StoreResponseDTO.ReviewPreViewListDTO> getMyReviewList(@CheckPage @RequestParam(name = "page", defaultValue = "1") Integer page) {
        Page<Review> reviews = memberQueryService.getMyReviewList(page - 1);
        return ApiResponse.onSuccess(StoreConverter.reviewPreViewListDTO(reviews));
    }

    @Operation(summary = "내가 진행 가능/진행 중/진행 완료한 미션 목록 조회 API",description = "내가 진행 가능/진행 중/진행 완료한 미션들의 목록을 조회하는 API이며, 페이징을 포함합니다. " +
                                                                                "query String 으로 status( none / progressing / completed )와 page 번호를 주세요")
    @GetMapping("/missions")
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "COMMON200",description = "OK, 성공"),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "AUTH003", description = "access 토큰을 주세요!",content = @Content(schema = @Schema(implementation = ApiResponse.class))),
    ...
    public ApiResponse<MissionResponseDTO.MyMissionPreViewListDTO> getMyMissionList(@RequestParam(name = "status") MissionStatus status,
                                                                                    @CheckPage @RequestParam(name = "page", defaultValue = "1") Integer page) {
        Page<MemberMission> memberMissionList = memberQueryService.getMyMissionList(page - 1, status);
        return ApiResponse.onSuccess(MemberMissionConverter.myMissionPreViewListDTO(memberMissionList));
    }
